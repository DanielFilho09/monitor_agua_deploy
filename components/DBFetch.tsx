      ...
      <View style={styles.container}>

        {
          ...
          bebedouros && bebedouros.map((item, index) => {
            ...
            let porcentagem = 100 - (item.volume ? ((100 * (item.volume)) / 20) : 0)
            let agua = 0
            switch (true) {
              case porcentagem <= 5:
                agua = 1000
              ...
              <View style={styles.tank} key={index}>
...

                {
                  item.volume && item.capacidade &&
                    <View style={{
                      ...styles.card,
                      backgroundColor: porcentagem <= 10 ? '#d1284f' : porcentagem <= 25 ? '#F68B36' : '#32BAFA',
                      borderColor: porcentagem <= 10 ? '#fa4069' : porcentagem <= 25 ? '#ffc994' : '#71CDF9'
                    }}>
                      <Text style={styles.percent}>{porcentagem}%</Text>
                      <Text style={styles.title}>{item.nome}</Text>
                      <Text style={styles.info}>Capacidade: 20l</Text>
